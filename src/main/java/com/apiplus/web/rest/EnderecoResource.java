...
import com.apiplus.domain.Endereco;
import com.apiplus.repository.EnderecoRepository;
...
/**
 * REST controller for managing {@link com.apiplus.domain.Endereco}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class EnderecoResource {
    ...
    public List<Endereco> getAllEnderecos(@RequestParam(required = false) String filter) {
        if ("parceiro-is-null".equals(filter)) {
            log.debug("REST request to get all Enderecos where parceiro is null");
            return StreamSupport
                .stream(enderecoRepository.findAll().spliterator(), false)
                .filter(endereco -> endereco.getParceiro() == null)
